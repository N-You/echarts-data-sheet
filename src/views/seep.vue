<template>
  <div v-if="step === 0" class="intro">
    <div>
      横向卡片最大平铺排数
      <input min="2" max="10" type="range" v-model="option.x" /> {{ option.x }}
    </div>
    <div>
      纵向卡片最大平铺排数
      <input min="2" max="10" type="range" v-model="option.y" /> {{ option.y }}
    </div>
    <div>
      卡片最大堆叠层数
      <input min="2" max="10" type="range" v-model="option.z" /> {{ option.z }}
    </div>
    <div>
      卡片密度
      <input
  ...
  <div v-else class="box">
    ...
    <div class="tools">
      道具：
      <button :disabled="!this.tools.save" @click="saveCard">
        取出3个卡片
      </button>
      <button :disabled="!this.tools.rand" @click="randCard">随机</button>
    </div>
  </div>
</template>
